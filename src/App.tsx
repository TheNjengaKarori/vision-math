...
import { Search, Calculator, Volume2 } from "lucide-react";
...
export default function App() {
  ...
  const [suggestion, setSuggestion] = useState<{ symbol: string; name: string; length: number; startC: number; } | null>(null);
  const suggestionRef = useRef(suggestion);
  const lastSpokenSuggestionRef = useRef<string | null>(null);

  const [computeStatus, setComputeStatus] = useState("");
  const [computedAnswers, setComputedAnswers] = useState<{ id: string; text: string; r: number; c: number; type: 'success' | 'error' }[]>([]);
  ...
  useEffect(() => {
    ...
    let word = "";
    while (c >= 0 && gridData[`${activeCell.r},${c}`] && gridData[`${activeCell.r},${c}`].trim() !== "") {
      word = gridData[`${activeCell.r},${c}`] + word;
      c--;
    }
    
    if (word.length >= 1) {
      const match = AUTOCOMPLETE_DICT.find(d => d.prefix.startsWith(word.toLowerCase()));
  ...
  const filteredSymbols = MATH_SYMBOLS.filter((s) => {
    ...
    return s.name.toLowerCase().includes(term) || s.symbol.toLowerCase().includes(term);
  });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!isCommandPaletteOpen && activeTag === "input" && activeId !== "grid-input") return;

      if (!isCommandPaletteOpen) {
        if (e.altKey && (e.key === "ArrowUp" || e.key === "ArrowDown" || e.key === "PageUp" || e.key === "PageDown")) {
          e.preventDefault();
          const isUp = e.key === "ArrowUp" || e.key === "PageUp";
          const newR = isUp ? Math.max(0, activeCellRef.current.r - 1) : activeCellRef.current.r + 1;
          setActiveCell({ r: newR, c: 0 });
          
          const lineBlocks = blocksRef.current.filter(b => b.r === newR).sort((a,b) => a.minC - b.minC);
          if (lineBlocks.length > 0) {
            speakText(`Line ${newR + 1}: ${lineBlocks.map(b => b.text).join(" ")}`);
          } else {
          ...
          if (suggestionRef.current) {
            ...
            setGridData((prev) => {
              ...
              for (let i = 0; i < sug.symbol.length; i++) {
                next[`${activeCellRef.current.r},${sug.startC + i}`] = sug.symbol[i];
              }
              return next;
            });
            setActiveCell({ r: activeCellRef.current.r, c: sug.startC + sug.symbol.length });
            speakText(sug.name);
            setSuggestion(null);
            setTimeout(() => document.getElementById("grid-input")?.focus(), 0);
            return;
          ...
          if (sortedBlocks.length > 0) {
            ...
            if (currentIndex !== -1) {
              ...
              if (nextIndex >= 0 && nextIndex < sortedBlocks.length) {
                ...
                nextBlock = sortedBlocks[dir === 1 ? 0 : sortedBlocks.length - 1];
              }
            } else {
              if (dir === 1) {
                nextBlock =
          ...
          if (!currentBlock) {
             const lineBlocks = blocksRef.current.filter(b => b.r === activeR).sort((a,b) => a.minC - b.minC);
             if (lineBlocks.length > 0) {
               currentBlock = { text: lineBlocks.map(b => b.text).join(""), maxC: lineBlocks[lineBlocks.length - 1].maxC } as any;
             }
          }

          if (currentBlock) {
            try {
              let textToEval = currentBlock.text;
              textToEval = textToEval.replace(/×/g, "*").replace(/÷/g, "/").replace(/√/g, "sqrt").replace(/π/g, "pi").replace(/∑/g, "sum").replace(/=/g, "");
              
              if (textToEval.trim() === "") return;
              const result = evaluate(textToEval);
              const formattedResult = (typeof result === 'number' && !Number.isInteger(result)) ? Number(result.toFixed(4)) : result;
  ...
  const activeRowKeys = Object.keys(gridData).map((k) => parseInt(k.split(",")[0], 10));
  const maxRowIndex = Math.max(activeCell.r, activeRowKeys.length > 0 ? Math.max(...activeRowKeys) : 0);
  ...
  const handleCellChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (matchedNemeth) {
      ...
      setGridData((prev) => {
        ...
        next[`${activeCell.r},${activeCell.c - len + 1}`] = matchedNemeth.symbol;
        return next;
      });
  ...
  const handlePaletteKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "ArrowDown") {
      ...
      scrollPaletteItemIntoView((paletteSelectedIndex + 1) % filteredSymbols.length);
    } else if (e.key === "ArrowUp") {
      e.preventDefault();
      setPaletteSelectedIndex((prev) => (prev - 1 + filteredSymbols.length) % filteredSymbols.length);
      scrollPaletteItemIntoView((paletteSelectedIndex - 1 + filteredSymbols.length) % filteredSymbols.length);
    } else if (e.key === "Enter") {
      e.preventDefault();
      if (filteredSymbols[paletteSelectedIndex]) {
        insertSymbol(filteredSymbols[paletteSelectedIndex].symbol);
      }
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden font-sans select-none relative bg-[#ffffff]">
      <div aria-live="polite" className="sr-only">
        {suggestion ? `Suggestion: ${suggestion.name}. Press Tab to accept.` : ''}
      </div>
        ...
        <div className="flex justify-between items-center h-14 bg-[#163e5b] text-white px-2 border-b-2 border-transparent relative z-20">
          ...
          <div className="flex-1 flex justify-end px-2">
            ...
            >
               ...
               <kbd className="hidden sm:inline-block bg-black/40 border border-white/5 rounded px-1.5 py-0.5 text-[10px] font-mono ml-1 text-white/70">Ctrl M</kbd>
            </button>
          </div>
        </div>
      </div>

      <div className="flex-1 flex flex-row pointer-events-auto h-full w-full relative z-0">
        <div className="flex-1 relative h-full overflow-y-auto">
          {/* Virtual Grid Layer as ordered list */}
          <ol
            className="list-decimal pl-[72px] pr-8 py-8 space-y-4 font-sans text-lg font-bold text-[#b4c9da] marker:text-[#86a8c3]"
          >
            {rowsList.map((r) => (
              <li key={r} className="relative min-h-[46px] w-[max-content] min-w-full cursor-text" onClick={(e) => handleRowClick(e, r)}>
                {/* The individual grid line layer */}
                <div
                  className="absolute inset-x-0 inset-y-0 z-0 pointer-events-none transition-colors duration-300 rounded"
                  style={
                    bgType === "grid" ? gridStyle : { backgroundColor: "transparent" }
                  }
                />
                <div className="relative z-10 h-[46px]">
                  {blocks.filter(b => b.r === r).map((block) => (
                    <div
                      key={block.id}
                      id={block.id}
                      role="group"
                      aria-label={`Equation: ${block.text}`}
                  ...
                  {activeCell.r === r && (
                    <input
                      ...
                      style={{
                        top: 0,
                        left: activeCell.c * 46,
                        width: 46,
                        height: 46,
                      }}
                      value={gridData[`${activeCell.r},${activeCell.c}`] || ""}
                  ...
                  {suggestion && activeCell.r === r && (
                    ...
                    >
                      <div className="flex items-baseline gap-2 mb-1.5">
                          <span className="text-white text-[18px] font-medium leading-none">{suggestion.symbol}</span>
                          <span className="text-[#b4c9da] text-xs font-light tracking-wide">{suggestion.name}</span>
                      </div>
                  ...
                  {computedAnswers.filter(a => a.r === r).map(answer => (
                    <div
                      key={answer.id}
                      ...
                      style={{
                        ...
                        left: answer.c * 46,
                        height: 46,
                        marginLeft: answer.type === 'success' ? 0 : 8 // small visual buffer for errors
                      }}
                    >
                      {answer.type === 'success' ? (
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh] sm:pt-[15vh] bg-[#112a3d]/60 backdrop-blur-sm p-4">
          ...
          <div className="relative w-full max-w-2xl bg-[#ffffff] rounded-2xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col max-h-[70vh]">
             <div className="flex items-center px-4 md:px-6 border-b border-gray-100 bg-white">
                <Search size={20} className="text-gray-400 mr-3 md:mr-4 shrink-0" />
                <input
                ...
                <kbd className="hidden sm:inline-block bg-gray-50 border border-gray-200 rounded-md px-2 py-1 text-xs font-mono text-gray-400 ml-1">ESC</kbd>
             </div>
             ...
             {filteredSymbols.length === 0 ? (
                ...
                <div ref={paletteListRef} className="flex-1 overflow-y-auto py-2 bg-gray-50/50">
                      ...
                      return (
                         ...
                         >
                            <div className="flex items-center gap-4 md:gap-5">
                               <span className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-lg text-2xl font-medium shrink-0 transition-colors
                            ...
                            {isSelected && (
                               <span className="hidden sm:flex text-xs text-blue-500 items-center gap-2">
                                  Press <kbd className="bg-blue-50 border border-blue-200 shadow-sm rounded-md px-2 py-1 uppercase tracking-wider text-[10px] font-bold text-blue-700">↵ Enter</kbd>
                               </span>
                            )}
                         </button>
                      );
